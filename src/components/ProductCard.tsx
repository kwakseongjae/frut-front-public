...
function ProductCard({
  ...
  const [isWished, setIsWished] = useState(product?.is_wished || false);
  const [isAnimating, setIsAnimating] = useState(false);
  const wishButtonRef = useRef<HTMLButtonElement>(null);
  const wishIconRef = useRef<HTMLDivElement>(null);

  const { contextSafe } = useGSAP({ scope: wishButtonRef });
  const addToWishlistMutation = useAddToWishlist();
  const deleteFromWishlistMutation = useDeleteFromWishlist();

  // product의 is_wished가 변경되면 상태 업데이트
  useEffect(() => {
    if (product?.is_wished !== undefined) {
      setIsWished(product.is_wished);
    }
  }, [product?.is_wished]);

  const handleWishClick = contextSafe(async (e: React.MouseEvent) => {
    e.preventDefault();
    e.stopPropagation();
    ...
    // 이미 찜한 상품이면 제거
    if (isWished) {
      if (!isAnimating && !deleteFromWishlistMutation.isPending) {
        try {
          await deleteFromWishlistMutation.mutateAsync(displayId);
          setIsWished(false);
        } catch (error) {
    ...
    // 찜하기 추가
    if (!isAnimating && !addToWishlistMutation.isPending) {
      setIsAnimating(true);

      try {
        // API 호출
        await addToWishlistMutation.mutateAsync(displayId);

        // 애니메이션 실행
        if (wishIconRef.current) {
        ...
        // 에러 발생 시 애니메이션 취소
        setIsAnimating(false);
        // 이미 찜한 상품인 경우 에러 메시지 표시 (선택사항)
        if (error instanceof Error) {
          console.error("찜하기 추가 실패:", error.message);
        }
      }
    }
  });
      ...
      <div className="w-full aspect-[1/1] relative bg-[#D9D9D9]">
        ...
        {displayRank !== undefined && displayRank !== null && (
          <div className="absolute top-0 left-0 w-8 h-8 bg-[#133A1B] flex items-center justify-center z-10">
            <span className="text-white font-bold text-sm">{displayRank}</span>
          </div>
        )}
        ...
        {!hideCartButton && (
          <div className="absolute right-3 bottom-3">
            ...
            >
              <div ref={wishIconRef}>
                {isAnimating || isWished ? <HeartGreenIcon /> : <HeartIcon />}
              </div>
            </button>
          </div>
        )}
