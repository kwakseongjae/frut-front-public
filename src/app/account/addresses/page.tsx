            ...
            <div className="px-5 py-4 space-y-4 pb-24">
              {addresses.map((address) => (
                <div
                  ...
                  role="button"
                  tabIndex={address.is_default ? -1 : 0}
                  aria-label={
                    address.is_default
                      ? `${address.recipient_name} 배송지 (기본 배송지)`
                      : `${address.recipient_name} 배송지 선택`
                  }
                  className={`border border-[#133A1B] rounded-lg p-4 bg-white transition-colors ${
                    selectedAddressId === address.id && !address.is_default
                      ? "bg-[#F7F7F7]"
                      : ""
                  } ${address.is_default ? "" : "cursor-pointer"}`}
                >
                  {/* 별칭 및 기본 배송지 태그 */}
