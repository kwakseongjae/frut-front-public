...
import { useInfiniteRefundableItems } from "@/lib/api/hooks/use-order";

const RefundPage = () => {
  const router = useRouter();
  const {
    data: refundableItemsData,
    isLoading,
    fetchNextPage,
    hasNextPage,
    isFetchingNextPage,
  } = useInfiniteRefundableItems();
  const observerTarget = useRef<HTMLDivElement>(null);

  // 모든 페이지의 데이터를 평탄화
  const refundableItems = useMemo(() => {
    if (!refundableItemsData?.pages) return [];
    return refundableItemsData.pages
