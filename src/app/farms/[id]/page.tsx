...
const NewsImageCarousel = ({ images, title }: NewsImageCarouselProps) => {
	...
	return (
		<div className="relative w-full aspect-[1/1] bg-[#D9D9D9] overflow-hidden rounded-lg">
			...
			{images.length > 1 && (
				<>
					<button
						type="button"
						onClick={scrollPrev}
						className="absolute left-2 top-1/2 transform -translate-y-1/2 w-8 h-8 bg-black/50 rounded-full flex items-center justify-center"
						aria-label="이전 이미지"
					>
					...
					<button
						type="button"
						onClick={scrollNext}
						className="absolute right-2 top-1/2 transform -translate-y-1/2 w-8 h-8 bg-black/50 rounded-full flex items-center justify-center"
						aria-label="다음 이미지"
					>
						<ChevronRightIcon />
					</button>
						...
						{review.images && review.images.length > 0 && (
							<div className="flex gap-2 overflow-x-auto">
								{review.images.map((image) => (
									<div
										key={image}
										className="relative w-20 h-20 rounded bg-[#D9D9D9] flex-shrink-0"
									>
										<Image
											src={image}
											alt={`${review.userName} 리뷰 이미지`}
											fill
